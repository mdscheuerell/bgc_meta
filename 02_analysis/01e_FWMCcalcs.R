...
solFW3 <- solFW2 %>% 
  ...
  # don't  need conc indicator since always mgL
  select(-conc) %>% 
  # make cols a factor
...
pdf(file.path(here::here("04_plots"),
              "01e_IntervalsBWconcSmp.pdf"), width = 10, height = 25)
...
# Export ----
write.csv(solM, file.path(here::here("03_generatedData"),
                         "01e_ProcessedConcQFwmc_AllSites.csv"))

save.image(file.path(here::here("07_Rdat"), "01ee_FWMCcalcs.Rdata"))
